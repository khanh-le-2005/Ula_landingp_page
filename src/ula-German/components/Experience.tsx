...
// --- DATA BÀI TẬP ĐA DẠNG (TIẾNG ĐỨC) ---
const EXERCISES = [
  {
    ...
    correct: 1,
    hint: "Hallo có nghĩa là Xin chào trong tiếng Đức.",
  ...
  },
  {
    ...
    pairs: [
      { id: "p1", left: "Danke", right: "Cảm ơn" },
      { id: "p2", left: "Ja.", right: "Có / Đúng" },
      { id: "p3", left: "Nein", right: "Không" },
    ],
  },
  {
    type: "fill",
    ...
    correct: "Vietnam",
    hint: "👉 Nghĩa: Tôi đến từ Việt Nam",
  },
  {
    type: "order",
    question: "Sắp xếp các từ thành câu đúng",
    target: "Ich heiße Linh.",
    words: ["heiße", "Ich", "Linh."],
    hint: "👉 Nghĩa: Tôi tên là Linh.",
  },
  {
    type: "listen",
    ...
    correct: 0,
    hint: "👉 Guten Morgen: Chào buổi sáng.",
    audio: true,
  },
  {
    type: "multiple_choice",
    question: "Muốn nói “Tôi học tiếng Đức.”",
    options: ["Ich lerne Deutsch.", "Ich komme Deutsch.", "Ich danke Deutsch."],
    correct: 0,
    hint: "👉 lerne = học.",
...
const Experience = () => {
  ...
  const [isMuted, setIsMuted] = useState(false); // <--- State bật tắt âm thanh
  ...
  const handleCheckAnswer = () => {
    ...
    if (ex.type === "multiple_choice" || ex.type === "listen" || ex.type === "mcq" || ex.type === "mcq_sentence") {
  ...
  const renderExerciseContent = () => {
            ...
            <div className="w-full md:w-5/12 flex-shrink-0 flex flex-col items-center justify-center bg-slate-50 rounded-2xl p-3 sm:p-4 border border-slate-100 relative">
              {ex.image && (
                <div className="relative w-full aspect-[4/3] sm:aspect-video md:aspect-auto md:h-full p-2">
                  <img src={ex.image} alt="exercise hint" className="absolute inset-0 w-full h-full object-contain object-center rounded-md drop-shadow-[0_12px_24px_rgba(26,43,72,0.14)]" />
                  {ex.audio && (
                    <button type="button" onClick={(e) => { e.stopPropagation(); playTTS(ex.audioText || ex.question); }} className="absolute inset-0 m-auto w-10 h-10 sm:w-12 sm:h-12 bg-[#0061ab]/90 text-white rounded-full flex items-center justify-center shadow-lg hover:scale-110 transition backdrop-blur-sm">
              ...
              {!ex.image && ex.audio && (
                <button type="button" onClick={(e) => { e.stopPropagation(); playTTS(ex.audioText || ex.question); }} className="w-16 h-16 sm:w-20 sm:h-20 bg-[#0061ab] text-white rounded-full flex items-center justify-center hover:scale-105 transition shadow-lg">
          ...
          <div className="w-full flex-1 flex flex-col justify-center">
            ...
            <div className={`grid ${ex.options && ex.options.length > 2 ? "grid-cols-1" : "grid-cols-2"} gap-3`}>
              {ex.options?.map((opt, idx) => {
                ...
                if (isChecking) {
                  if (idx === ex.correct) btnClass = "border-green-500 bg-green-50 text-green-700";
                  else if (isSelected) btnClass = "border-red-500 bg-red-50 text-red-700";
                } else if (isSelected) btnClass = "border-[#1a2b48] bg-blue-50 ring-1 ring-[#1a2b48]";

                const displayOpt = opt.startsWith("A.") || opt.startsWith("B.") || opt.startsWith("C.") ? opt.substring(3) : opt;

                return (
                  <button type="button" key={idx} onClick={() => !isChecking && setSelectedOpt(idx)} disabled={isChecking} className={`w-full p-4 rounded-xl border-2 text-left font-bold transition-all flex items-center gap-3 ${btnClass}`}>
    ...
    if (ex.type === "match") {
      return (
        <MatchingExercise ex={ex} userMatches={userMatches} isChecking={isChecking} matchRightItems={matchRightItems} selectedLeftId={selectedLeftId} selectedRightId={selectedRightId} handleSelectLeft={handleSelectLeft} handleSelectRight={handleSelectRight} />
      );
    }
    ...
    if (ex.type === "order") {
      return (
        <div className="w-full bg-white/30 backdrop-blur-md text-[#1a2b48] shadow-sm p-4 md:p-6 border border-slate-200 rounded-2xl">
          ...
          <p className="text-slate-500 font-medium text-sm mb-6 border-b pb-4">{ex.question}</p>
          <div className="min-h-[80px] bg-slate-50 p-4 md:p-6 rounded-2xl flex flex-wrap gap-2 md:gap-3 mb-4 border-2 border-dashed border-slate-300 items-center justify-center shadow-inner">
            {orderedWords.length === 0 && <span className="text-slate-400 text-sm font-medium">Bấm vào từ bên dưới để ghép câu...</span>}
            {orderedWords.map((w, idx) => (
              <button type="button" key={idx} disabled={isChecking} onClick={() => { setOrderedWords(orderedWords.filter((_, i) => i !== idx)); setAvailableWords([...availableWords, w]); setIsChecking(false); setIsCorrect(null); }} className="bg-[#0061ab] text-white px-5 py-2.5 rounded-xl font-bold shadow-md hover:bg-blue-700 transition-colors animate-in zoom-in duration-200">
                {w.text}
              </button>
            ))}
          </div>
          <div className="flex flex-wrap gap-3 justify-center border-t border-slate-100 pt-6">
            {availableWords.map((w, idx) => (
              <button type="button" key={idx} disabled={isChecking} onClick={() => { setAvailableWords(availableWords.filter((_, i) => i !== idx)); setOrderedWords([...orderedWords, w]); setIsChecking(false); setIsCorrect(null); }} className="bg-white border-2 border-slate-200 text-slate-700 px-5 py-2.5 rounded-xl font-bold shadow-sm hover:border-[#0061ab] hover:text-[#0061ab] transition-all hover:-translate-y-1">
                {w.text}
              </button>
            ))}
          </div>
    ...
    if (ex.type === "fill") {
      ...
      return (
        <div className="w-full bg-white/30 backdrop-blur-md text-[#1a2b48] shadow-sm p-4 md:p-6 border border-slate-200 rounded-2xl">
          <h3 className="text-lg font-black uppercase mb-6 border-b pb-4 whitespace-pre-line">{ex.question}</h3>
            ...
            {sentenceParts.map((part, i) => (
              <React.Fragment key={i}>
                ...
                {i < sentenceParts.length - 1 && (
                  <input type="text" value={fillAnswer} onChange={(e) => { setFillAnswer(e.target.value); setIsChecking(false); setIsCorrect(null); }} disabled={isChecking} className={`mx-2 px-3 py-1 border-b-2 rounded-lg outline-none w-32 text-center transition-colors ${isChecking ? (isCorrect ? "border-green-500 text-green-600 bg-green-50 rounded-lg" : "border-red-500 text-red-600 bg-red-50 rounded-lg") : "border-[#dfc38a] focus:border-[#1a2b48]"}`} />
                )}
              </React.Fragment>
            ))}
          </div>
          {ex.hint && <p className="text-sm text-slate-500 mt-6 italic text-center">{ex.hint}</p>}
        </div>
      );
    }
    return null;
          ...
          <>
            {isCompleted ? (
              <div className="animate-in zoom-in flex flex-col items-center justify-center h-full min-h-[500px] text-center p-8">
                ...
                <p className="text-slate-500 font-medium text-sm md:text-base mb-8">Thật dễ dàng đúng không? Học tiếng Đức không hề khó khi có lộ trình chuẩn.</p>
                  ...
                  {!isChecking ? (
                    <button
                      ...
                      disabled={
                        ...
                        (EXERCISES[currentEx].type === "match" && userMatches.length !== EXERCISES[currentEx].pairs?.length) ||
