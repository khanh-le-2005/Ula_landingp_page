...
import { Mic, BookOpen, PlayCircle, Volume2, VolumeX, CheckCircle2, XCircle, ArrowRight, RotateCcw, Check, X } from 'lucide-react';
...
const EXERCISES = [
  ...
  },
  {
    ...
    correct: 0,
    hint: "👉 Người học thấy phát âm thú vị.",
    audio: true,
  },
  {
    type: "multiple_choice",
    ...
    const newLines = userMatches
      .map((match) => {
        ...
        if (leftDot && rightDot) {
          ...
          if (isChecking) {
            // Kiểm tra xem user có nối đúng với file gốc không
            const isCorrectMatch = ex.pairs?.some(p => (p.left || p.zh) === match.left && (p.right || p.vi) === match.right);
      ...
      >
        ...
        <div className="space-y-4 relative z-20 flex flex-col justify-center">
          {ex.pairs?.map((item, index) => {
            const leftText = item.left || item.zh || '';
        ...
        <div className="space-y-4 relative z-20 flex flex-col justify-center">
          {matchRightItems?.map((item, index) => {
            const rightText = item.right || item.vi || '';
...
const Experience = () => {
  ...
  // --- STATE CHO BÀI TẬP MỚI (Lấy từ ExerciseDemo) ---
  ...
  // --- CÁC HÀM XỬ LÝ BÀI TẬP TỪ EXERCISEDEMO ---
  useEffect(() => {
    ...
    if (
      ...
      if (ex.pairs) {
        correct =
          ...
          userMatches.every((m) => ex.pairs?.some(p => (p.left || p.zh) === m.left && (p.right || p.vi) === m.right));
      }
  ...
  return (
    <section id="experience" className="flex flex-col items-center py-12 px-4 font-sans relative overflow-hidden reveal">
      ...
      <div className="w-full max-w-5xl bg-white/30 backdrop-blur-2xl rounded-[32px] md:rounded-[40px] overflow-hidden min-h-[50px] md:min-h-[600px] flex flex-col relative z-10 reveal shadow-xl border border-white/50">
        ...
        {/* NỘI DUNG TAB: VIDEO (Giữ nguyên của bạn) */}
        {activeTab === 'video' && (
        ...
        {/* NỘI DUNG TAB: AI PHÁT ÂM (Giữ nguyên của bạn) */}
        {activeTab === 'pronounce' && (
