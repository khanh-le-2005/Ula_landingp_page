...
const FeatureCard = ({ number, title, subTitle, imgSrc }: { number: string; title: string; subTitle: string; imgSrc: string }) => (
  <div className="bg-white/60 rounded-[24px] sm:rounded-[32px] p-4 sm:p-6 border border-white/60 flex flex-col items-center text-center group transition-all duration-500 shadow-sm font-extrabold font-be-vietnam-pro">
    <span className="text-[10px] sm:text-[12px] font-extrabold text-slate-400 uppercase tracking-widest transition-colors ">{number}</span>
    ...
    <div className="mt-3 sm:mt-4">
      <h3 className="text-sm sm:text-lg font-extrabold text-slate-700 leading-tight transition-colors uppercase">{title}</h3>
...
const Methodology = () => {
  ...
  return (
    <section className=" p-4 sm:p-6 lg:p-12 font-extrabold font-be-vietnam-pro text-slate-800 flex items-center justify-center reveal" id="methodology">
      <div className="max-w-7xl w-full mx-auto grid grid-cols-12 gap-4 sm:gap-6 items-stretch">
        ...
        <div className=" col-span-12 lg:col-span-7 bg-white/50 rounded-[32px] sm:rounded-[40px] py-8 sm:py-10 flex flex-col shadow-sm px-4">
          <span className="text-[14px] sm:text-[14px] font-extrabold text-black uppercase tracking-widest px-5 text-center w-full">{mainCard.number}</span>
