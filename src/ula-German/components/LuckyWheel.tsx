...
export default function LuckyWheel({ onWin }: { onWin: (prize: { option: string; code: string }) => void }) {
  ...
  return (
    <section className="py-4 md:py-8 text-white rounded-[2rem] md:rounded-[3rem] bg-white mx-4 md:mx-6 mb-20 md:mb-24 overflow-hidden relative reveal" id="luckywheel">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-12 items-center relative z-10">
        <div className="space-y-6 md:space-y-8 text-center md:text-left">
          ...
          <h2 className="text-3xl sm:text-4xl md:text-6xl font-black leading-[1.1] text-slate-900 tracking-tight">
            ...
            <span className="inline-block text-[1.08em] sm:text-[1.1em] lg:text-[1.08em] text-[#d4b36d] drop-shadow-[0_6px_16px_rgba(197,160,89,0.14)] [-webkit-text-stroke:0.5px_#ffffcc] ">
          ...
          <p className="text-lg md:text-xl text-slate-600 max-w-2xl mx-auto md:mx-0 leading-relaxed font-medium">
            Nhập mã <span className="text-secondary font-black px-2 py-0.5 rounded bg-red-50 border border-red-100">ULA40GER</span> để nhận ưu đãi 40% cho khoá lộ trình từ A1 - B1 và thêm quà tặng từ vòng quay may mắn.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center md:justify-start">
            <a className="bg-secondary text-white px-8 py-3.5 md:py-4 rounded-full font-bold text-center hover:opacity-90 transition-all hover:cursor-pointer text-[24px] md:text-base" href="#lead-form" style={{ fontSize: '24px' }}>Giữ ưu đãi ngay</a>
