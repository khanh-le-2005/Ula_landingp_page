          ...
          <div className="relative w-full max-w-2xl overflow-hidden rounded-[40px] border border-slate-200 bg-white shadow-2xl animate-in zoom-in-95 duration-300">
            <div className="p-8 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-xl font-black text-slate-900 tracking-tight">Cấp phát Link tự động</h3>
                ...
                <div className="space-y-4">
                  <div className="mb-6 p-4 rounded-2xl bg-indigo-50/50 border border-indigo-100 flex gap-4 items-center">
                    <div className="flex-1">
                      ...
                      <div className="font-bold text-slate-900 text-lg">{utmLinksData.affiliateName}</div>
                    </div>
                    <div className="flex-1">
                      <div className="text-[10px] font-black uppercase tracking-widest text-indigo-500 mb-1">Mã Tracking (Ref)</div>
                      <div className="font-mono font-bold text-slate-900 text-lg">{utmLinksData.referralCode}</div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    {utmLinksData.links.map((link, idx) => (
                      <div key={idx} className="flex flex-col sm:flex-row gap-3 items-start sm:items-center p-4 rounded-2xl border border-slate-100 hover:border-indigo-100 hover:bg-slate-50 transition-colors">
                        <div className="w-28 shrink-0 font-black text-xs uppercase tracking-widest text-indigo-600 bg-indigo-50 px-3 py-1.5 rounded-lg text-center">
                          {link.platform}
                        </div>
                        <div className="flex-1 w-full bg-white px-4 py-2.5 rounded-xl text-xs font-medium text-slate-500 truncate lowercase border border-slate-200 shadow-sm">
                          {link.url}
                        </div>
                        <button
                          onClick={() => handleCopyAnyLink(link.url, idx)}
                          className={`h-10 px-5 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-2 ${copiedCode === 'link-' + idx
