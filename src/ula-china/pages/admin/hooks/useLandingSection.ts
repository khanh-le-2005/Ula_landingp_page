...
export function useLandingSection<T>(sectionKey: string, fallback: T) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const landingPage = await fetchLandingPage(isAdmin ? undefined : siteKey);
  ...
  const save = async (nextContent?: T) => {
    ...
    try {
      const response = await updateLandingSection(sectionKey, value, isAdmin ? undefined : siteKey);
