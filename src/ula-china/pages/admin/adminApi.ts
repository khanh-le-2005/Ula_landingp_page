...
const getSiteContext = (siteOverride?: string, variantOverride?: string) => {
  if (typeof window === 'undefined') return { site: siteOverride || 'tieng-trung', variant: variantOverride || 'default' };
  ...
  if (path.startsWith('/admin') && storedProject) {
    return { site: storedProject, variant: params.get('variant') || 'default' };
  }

  // 2. Kiểm tra đường dẫn URL (vd: /china, /german) cho trang khách hàng
...
export const fetchAffiliateLinks = async (id: string) => {
  const token = getStoredAdminToken();
  return requestJson<AffiliateLinksResponse>(`/api/affiliates/${id}/links`, { method: 'GET' }, {}, token);
};
