...
const MatchingExercise = ({ data, isChecked = false, onResult = () => { } }: MatchingExerciseProps) => {
  ...
  const updateLayout = useCallback(() => {
    ...
    data.pairs.forEach((pair) => {
      const leftDot = leftDotRefs.current[pair.zh];
  ...
  useLayoutEffect(() => {
    updateLayout();
  }, [updateLayout, connections, selectedZh, isChecked]);
  ...
  const handleSelect = (type: 'zh' | 'vi', value: string) => {
    if (isChecked) return;

    if (type === 'zh') {
      const existing = connections.find((c) => c.zh === value);
      if (existing) {
        setConnections((prev) => prev.filter((c) => c.zh !== value));
        setSelectedZh(null);
        return;
      }
      setSelectedZh(value);
    ...
    setConnections((prev) => {
      ...
      const pairColor = data.pairs.find((p) => p.zh === selectedZh)?.color || DEFAULT_COLOR;
      const next = [...filtered, { zh: selectedZh, vi: value, color: pairColor }];
      const isAllCorrect =
        next.length === data.pairs.length &&
        next.every((c) => data.pairs.find((p) => p.zh === c.zh)?.vi === c.vi);
    ...
    return connections
      .map((conn) => {
        ...
        const strokeColor = isChecked
          ? data.pairs.find((p) => p.zh === conn.zh)?.vi === conn.vi
    ...
    >
      ...
      <div ref={containerRef} className="relative grid grid-cols-[1fr_1fr] gap-x-12 sm:gap-x-24 md:gap-x-32 gap-y-4 md:gap-y-6">
        <div className="relative z-20 flex flex-col gap-6">
          {data.pairs.map((pair) => (
            <div key={pair.zh} className="relative flex items-center">
              <button
                onClick={() => handleSelect('zh', pair.zh)}
                className={`flex-1 rounded-2xl border-2 px-4 py-3 sm:px-8 sm:py-5 text-sm sm:text-lg font-bold transition-all ${selectedZh === pair.zh
              ...
              <div
                ref={(node) => {
                  leftDotRefs.current[pair.zh] = node;
                }}
                className={`absolute -right-3 h-5 w-5 rounded-full border-4 border-white bg-slate-200 shadow-md ${connections.some((conn) => conn.zh === pair.zh) ? 'bg-blue-400' : ''
                  }`}
              />
            </div>
          ))}
